...
namespace Đồ_án
{
    public partial class ReportFull : Form
    {
        ...
        Database db;
        public int tuan, thang, nam,choise;
        ...
        private void ReportFull_Load(object sender, EventArgs e)
        {
            ...
            if (choise == 0)
                sql = "select a.MaNH,a.Name,isnull(a.SLW,0) as 'SW',isnull(a.SLM,0) as 'SM',isnull(c.TM,0) as 'SY' from (select aa.MaNH,aa.Name,aa.SL as 'SLW',bb.SL as 'SLM' from " +
            ...
            if(choise==1)
                ...
                "(select N.MaNH, N.Name, W.SL from NGUOIHOC N left join (select MaNH, SL from TKEWEEK where TUAN = '" + tuan + "' and NAM = '" + nam + "') W on N.MaNH = W.MaNH) aa " +
                "left join(select MaNH, SL, THANG, NAM from TKEMONTH where THANG = '" + thang + "' and NAM = '" + nam + "' ) bb on aa.MaNH = bb.MaNH) a " +
                "left join(select M.MaNH, SUM(M.SL) as 'TM' from TKEMONTH M where NAM = '" + nam + "' group by MaNH) c on c.MaNH = a.MaNH where "+maND;
